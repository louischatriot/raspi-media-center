...
movies.setAutocompactionInterval(config.autocompactionInterval);
  ...
  function checkIfActive () {
    ...
    try {
      ...
      movies.findOne({ path: self.currentlyPlaying }, function (err, movie) {
        // If error don't fire it, simply start playback from the beginning
        if (!err && movie) {
          player.setPosition(Math.floor(movie.position / 1000));
        }
        return callback();
      });
    } catch (e) {
